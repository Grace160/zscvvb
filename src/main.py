...
from models import Clipper, fMRIEncoderCross, fMRIEncoderSpecific
...
def prepare_voxel2clip(args, out_dim_image, out_dim_text, device):
    ...
    if len(args.subj_list) == 1: # Single subject does not need "brain builder"
        ...
        voxel2clip = fMRIEncoderSpecific(**voxel2clip_kwargs).to(device)
    else:
        voxel2clip = fMRIEncoderCross(**voxel2clip_kwargs).to(device)
...
def   prepare_trainer(args, accelerator, voxel2clip, clip_extractor, prompts_list, device):
    if args.adapting:
        ...
        trainer = Trainer_specific(args, accelerator, voxel2clip, clip_extractor, prompts_list, device)
    else:
        trainer = Trainer_cross(args, accelerator, voxel2clip, clip_extractor, prompts_list, device)
