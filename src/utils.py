...
def reconstruction(
    ...
    if unet is not None:
        ...
        # print(len(vae.config.block_out_channels))#4
        vae_scale_factor = 2 ** (len(vae.config.block_out_channels) - 1)#8
        height = unet.config.sample_size * vae_scale_factor#512
        width = unet.config.sample_size * vae_scale_factor#512
    ...
    if voxel2clip is not None:
        ...
        brain_clip_image_embeddings = brain_clip_image_embeddings.repeat(recons_per_sample, 1, 1)#8,257,768

        brain_clip_text_embeddings  = brain_clip_text_embeddings.repeat(recons_per_sample, 1, 1)

    if recons_per_sample > 0:
        for samp in range(len(brain_clip_image_embeddings)):#8
        ...
        if do_classifier_free_guidance:
            input_embedding = torch.cat([torch.zeros_like(input_embedding), input_embedding]).to(device).to(unet.dtype)#16 257 768
            prompt_embeds = torch.cat([torch.zeros_like(prompt_embeds), prompt_embeds]).to(device).to(unet.dtype)#16 77 768
        ...
        if img_lowlevel is not None: # use img_lowlevel for img2img initialization
            ...
            # print("走这里吗")
            timesteps = noise_scheduler.timesteps#代表什么
            # print(timesteps)
            latents = torch.randn([recons_per_sample, 4, 64, 64], device=device,
                                  generator=generator, dtype=input_embedding.dtype)#潜在空间是随机的
    ...
    for im in range(recons_per_sample): 
        currecon = clip_extractor.embed_image(brain_recons[0,[im]].float()).to(clip_image_target_norm.device).to(clip_image_target_norm.dtype)#重新输入clip中计算特征
