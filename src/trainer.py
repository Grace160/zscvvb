...
class RKdAngle(nn.Module):
    def forward(self, teacher, student):
        with torch.no_grad():
            # teacher 的角度关系
            teacher = F.normalize(teacher, p=2, dim=-1)  # (50, 257, 768)
            t_angle = torch.bmm(teacher, teacher.transpose(1, 2))

        # student 的角度关系
        student = F.normalize(student, p=2, dim=-1)  # (50, 257, 768)
        s_angle = torch.bmm(student, student.transpose(1, 2))

        # 计算 smooth L1 损失
        loss = F.smooth_l1_loss(s_angle, t_angle, reduction='mean')
        # loss = F.mse(s_angle, t_angle, reduction='mean')
...
class Trainer:
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
        ...
        self.instance_relation_loss = RKdAngle()
    ...
    def train(self, local_rank):
        ...
        for epoch in progress_bar:
            ...
            self.val_loss_clip_image_context_sum = 0.
            self.val_loss_mse_image_context_sum = 0.
            self.val_loss_clip_text_context_sum = 0.
            self.val_loss_mse_text_context_sum = 0.
            self.loss_clip_image_context_sum = 0.
            self.loss_clip_text_context_sum = 0.
            self.loss_mse_image_context_sum = 0.
            self.loss_mse_text_context_sum = 0.
            # wandb logging
    ...
    def train_step(self, voxel, image, captions, subj_id):
        ...
        if self.args.use_image_aug:
            ...
            image = data.img_augment_onlyresize(image)

        clip_image = self.clip_extractor.embed_image(image).float()
        clip_text = self.clip_extractor.embed_text(captions).float()

        ##add
        batch_size, num_image_features, feature_dim = clip_image.shape
        cluster_image_centers_tensor = torch.empty(batch_size, self.args.K_image, feature_dim).cuda()
        # cluster_image_labels_tensor = torch.empty(batch_size, K_image).cuda()
        group_pseudo_label_clip_image = torch.zeros_like(clip_image).cuda()

        _, num_text_features, _ = clip_text.shape
        cluster_text_centers_tensor = torch.empty(batch_size, self.args.K_text, feature_dim).cuda()
        # cluster_text_labels_tensor = torch.empty(batch_size, K_text).cuda()
        group_pseudo_label_clip_text = torch.zeros_like(clip_text).cuda()

        for i in range(batch_size):
            batch_image_features_a = clip_image[i]
            flat_image_features_a = batch_image_features_a.view(num_image_features, feature_dim)
            image_kmeans = KMeans(n_clusters=self.args.K_image, random_state=0)
            image_cluster_centers = image_kmeans.fit(flat_image_features_a.cpu().numpy()).cluster_centers_
            cluster_image_centers_tensor[i] = torch.tensor(image_cluster_centers)
            # cluster_image_labels_tensor[i] = torch.tensor(image_kmeans.labels_)
            group_pseudo_label_clip_image[i] = torch.tensor(image_cluster_centers[image_kmeans.labels_])

            batch_text_features_a = clip_text[i]
            flat_text_features_a = batch_text_features_a.view(num_text_features, feature_dim)
            text_kmeans = KMeans(n_clusters=self.args.K_text, random_state=0)
            text_cluster_centers = text_kmeans.fit(flat_text_features_a.cpu().numpy()).cluster_centers_
            cluster_text_centers_tensor[i] = torch.tensor(text_cluster_centers)
            # cluster_text_labels_tensor[i] = torch.tensor(text_kmeans.labels_)
            group_pseudo_label_clip_text[i] = torch.tensor(text_cluster_centers[text_kmeans.labels_])

        clip_image_context_norm = nn.functional.normalize(group_pseudo_label_clip_image.flatten(1), dim=-1)
        clip_text_context_norm = nn.functional.normalize(group_pseudo_label_clip_text.flatten(1), dim=-1)
        ...
        # image clip loss
        clip_image_pred = results[0]  # torch.Size([50, 197376])
        ...
        # # ##add
        clip_image_context_pred_norm = nn.functional.normalize(clip_image_pred.flatten(1), dim=-1)
        #
        loss_clip_image_context = utils.soft_clip_loss(
            clip_image_context_pred_norm,
            clip_image_context_norm,
        )
        loss += loss_clip_image_context
        self.loss_clip_image_context_sum += loss_clip_image_context.item()

        # rkd_image_loss = RKdAngle()(clip_image, clip_image_pred.view(batch_size, num_image_features, feature_dim))
        # loss += rkd_image_loss
        instance_image_relation_loss = self.instance_relation_loss(clip_image,
                                                                   clip_image_pred.view(batch_size, num_image_features,
                                                                                        feature_dim))
        loss += instance_image_relation_loss

        # image mse loss
        if self.args.mse_mult:
            # print(clip_image_pred_norm.size()) #torch.Size([50, 197376])  bs50
            ...
            ##add
            loss_mse_image_context = nn.MSELoss()(clip_image_context_pred_norm, clip_image_context_norm)
            loss += self.args.mse_mult * loss_mse_image_context
            self.loss_mse_image_context_sum += loss_mse_image_context.item()

        # text clip loss
        clip_text_pred = results[1]
        # print("clip_text_pred.size()",clip_text_pred.size()) clip_text_pred.size() torch.Size([50, 59136])
        ...
        #add
        clip_text_context_pred_norm = nn.functional.normalize(clip_text_pred.flatten(1), dim=-1)
        loss_clip_text_context = utils.soft_clip_loss(
            clip_text_context_pred_norm,
            clip_text_context_norm,
        )
        loss += loss_clip_text_context
        self.loss_clip_text_context_sum += loss_clip_text_context.item()

        # rkd_text_loss = RKdAngle()(clip_text, clip_text_pred.view(batch_size, num_text_features, feature_dim))
        # loss += rkd_text_loss

        instance_text_relation_loss = self.instance_relation_loss(clip_text,
                                                                  clip_text_pred.view(batch_size, num_text_features,
                                                                                      feature_dim))
        loss += instance_text_relation_loss
        ...
        if self.args.mse_mult:
            ...
            ##add
            loss_mse_text_context = nn.MSELoss()(clip_text_context_pred_norm, clip_text_context_norm)
            loss += self.args.mse_mult * loss_mse_text_context
            self.loss_mse_text_context_sum += loss_mse_text_context.item()
        ...
        self.fwd_percent_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_pred_norm, clip_image_norm),
                                               labels, k=1)
        self.bwd_percent_correct += utils.topk(utils.batchwise_cosine_similarity(clip_image_norm, clip_image_pred_norm),
                                               labels, k=1)
    ...
    def eval_step(self, voxel, image, captions, subj_id):
        ...
        with torch.no_grad():
            ...
            # # ##add
            # clip_image_context_pred = results[2]
            # clip_image_context_pred_norm = nn.functional.normalize(clip_image_context_pred.flatten(1), dim=-1)
            # clip_image_context_norm = self.context_feature(clip_image)
            # val_loss_clip_image_context = utils.soft_clip_loss(
            #     clip_image_context_pred_norm,
            #     clip_image_context_norm,
            # )
            # val_loss += val_loss_clip_image_context
            # self.val_loss_clip_image_context_sum += val_loss_clip_image_context.item()
            ...
            if self.args.mse_mult:
                ...
                # ##add
                # val_loss_mse_image_context = nn.MSELoss()(clip_image_context_pred_norm, clip_image_context_norm)
                # val_loss += self.args.mse_mult * val_loss_mse_image
                # self.val_loss_mse_image_context_sum += val_loss_mse_image_context.item()
            ...
            # # ##add
            # clip_text_context_pred = results[3]
            # clip_text_context_pred_norm = nn.functional.normalize(clip_text_context_pred.flatten(1), dim=-1)
            # clip_text_context_norm = self.context_feature(clip_text)
            # val_loss_clip_text_context = utils.soft_clip_loss(
            #     clip_text_context_pred_norm,
            #     clip_text_context_norm,
            # )
            # val_loss += val_loss_clip_text_context
            # self.val_loss_clip_text_context_sum += val_loss_clip_text_context.item()
            ...
            if self.args.mse_mult:
                ...
                # ##add
                # val_loss_mse_text_context = nn.MSELoss()(clip_text_context_pred_norm, clip_text_context_norm)
                # val_loss += self.args.mse_mult * val_loss_mse_text_context
                # self.val_loss_mse_text_context_sum += val_loss_mse_text_context.item()
            ...
            labels = torch.arange(len(clip_image_norm)).to(self.device)
            self.val_fwd_percent_correct += utils.topk(
                utils.batchwise_cosine_similarity(clip_image_pred_norm, clip_image_norm), labels, k=1)
            self.val_bwd_percent_correct += utils.topk(
                utils.batchwise_cosine_similarity(clip_image_norm, clip_image_pred_norm), labels, k=1)
...
class Trainer_specific(Trainer):
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
...
class Trainer_adapt(Trainer):
    ...
    def train_epoch(self, epoch):
        ...
        for train_i, datas_target in enumerate(self.train_dl_target):
            ...
            print(">>> Epoch{} | Iter{} | source{} | voxel: {}".format(epoch, train_i, source_index, voxel.shape),
                  flush=True)
            self.train_step(voxel, image, captions, subj_id)
