...
import utils
from models import Clipper,  fMRIEncoderCross, fMRIEncoderSpecific, Voxel2StableDiffusionModel
import data
...
def prepare_VD(args, device):
    ...
    try:
        ...
        vd_pipe =  VersatileDiffusionDualGuidedPipeline.from_pretrained(
                "/nas/sunguoying/pretrained/versatile-diffusion",
                cache_dir = args.vd_cache_dir)
    ...
    vd_pipe.scheduler = UniPCMultistepScheduler.from_pretrained("/nas/sunguoying/pretrained/versatile-diffusion", cache_dir=args.vd_cache_dir, subfolder="scheduler")
...
def prepare_voxel2clip(args, out_dim_image, out_dim_text, device):
    voxel2clip_kwargs = dict(
        ...
        h=args.h_size, n_blocks=args.n_blocks, subj_list=args.subj_load)#subj_load控制模型结构

    voxel2clip = fMRIEncoderSpecific(**voxel2clip_kwargs)
...
def main(device):
    ...
    if args.subj_load is None:
        args.subj_load = [args.subj_test]#subj_test控制加载数据
    ...
    ckpt_path = os.path.join(outdir_ae, f'epoch120.pth') #没有使用
    ...
    outdir = f'../train_logs/{args.model_name}'
    save_dir = os.path.join(outdir, f"recon_{args.ckpt_from}_on_subj{args.subj_test}")
        ...
        with torch.no_grad():
            ...
            if only_lowlevel: # only low-level pipeline
                ...
                grid, brain_recons, best_picks, recon_img = utils.reconstruction(
                    ...
                    guidance_scale = args.guidance_scale,#和guide的区别是什么
